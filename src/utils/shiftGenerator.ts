...
export function generateShifts(
  ...
  // 0.5 Enforce Night-PostNight-Off (夜-明-休) for pre-assigned night shifts
  targetStaff.forEach(staff => {
    dates.forEach((date, index) => {
      const dateStr = formatDate(date);
      if (getShift(staff.id, dateStr) === '夜') {
        if (index + 1 < dates.length) {
          const nextDateStr = formatDate(dates[index + 1]);
          setShift(staff.id, nextDateStr, '明', true);
        }
        if (index + 2 < dates.length) {
          const nextNextDateStr = formatDate(dates[index + 2]);
          setShift(staff.id, nextNextDateStr, '-', true);
        }
      }
    });
  });
      ...
      if (staff.specialRules.includes('金曜夜勤のみ')) {
        ...
        if (dayOfWeek === 5) {
          ...
          if (current !== '有') {
            ...
            if (sat <= dates[dates.length-1]) setShift(staff.id, formatDate(sat), '明', true);
            if (sun <= dates[dates.length-1]) setShift(staff.id, formatDate(sun), '-', true);
          }
        }
      }
    });
